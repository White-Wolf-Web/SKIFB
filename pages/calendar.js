...
import FilterContainer from "../components/calendar/CalendarFilter";
...
export default function Calendar(props) {
	...
	return (
		<main>
			...
			<div className={styles.calendarReact}>
				{loading ? (
					<Loader />
				) : (
					<div className={styles.calendarAccordion}>
						{filteredItems.map((item) => (
							<Accordion defaultActiveKey="false" key={item.id} className={styles.accordion}  flush>
							<Accordion.Item eventKey="0" className={`${styles.accordion} ${styles.accordionItem}`} >
							<Accordion.Header className={`${styles.accordionHeader} ${styles.accordionGreenArrow}`}>
										<div className={styles.calendarAccordionHeader}  >
											<div> {item.startDay + "/" + item.startMonth + "/" + item.startYear} </div>
											<div>
												{" "}
												<span className={styles.breakingSpace}></span> {item.title}
											</div>
										</div>
									</Accordion.Header>
									<Accordion.Body className={`${styles.calendarBody} ${styles.accordionBody}`}>
										<div className={styles.calendarCardText}>
											...
											<div>
												<div className={styles.br}>{item.price}</div>
											</div>
...
export async function getStaticProps({ locale }) {
	return {
		props: {
			...(await serverSideTranslations(locale, ["calendar"])),
		},
	};
}
